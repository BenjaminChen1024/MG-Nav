...
# 定义您的目标保存目录
SAVE_DIR = "/nas_home/wangbo/Navigation_Record/navdp_baseline/navdp_videos"
# 确保目录存在
...
def navdp_reset():
    ...
    if navdp_fps_writer is None:
        ...
        file_path = os.path.join(SAVE_DIR, "{}_fps_pointgoal.mp4".format(format_time))
        navdp_fps_writer = imageio.get_writer(file_path, fps=7)
    else:
