...
class NavDP_Policy_Geometry(nn.Module):
    def __init__(self,
                 image_size=224,
                 memory_size=8,
                 predict_size=24,
        ...
        # 新增
        self.geometry_encoder = NavDP_Geometry_Backbone(image_size, token_dim, device=device)
        ...
        # 新增
        geometry_token_num = self.geometry_encoder.token_num
        self.geometry_pos_embed = LearnablePositionalEncoding(self.token_dim, geometry_token_num)
    ...
    def predict_noise_geom(self,last_actions,timestep,goal_embed,rgbd_embed, geom_embed):
        action_embeds = self.input_embed(last_actions)
        time_embeds = self.time_emb(timestep.to(self.device)).unsqueeze(1).tile((last_actions.shape[0],1,1))

        # 新增
        geom_embed_pos = self.geometry_pos_embed(geom_embed)
        geom_embedding = geom_embed + geom_embed_pos

        cond_embedding = torch.cat([time_embeds,goal_embed,goal_embed,goal_embed,rgbd_embed],dim=1) + self.cond_pos_embed(torch.cat([time_embeds,goal_embed,goal_embed,goal_embed,rgbd_embed],dim=1))
        
        # 新增
        cond_embedding_geom = torch.cat([cond_embedding, geom_embedding], dim=1)
        
        input_embedding = action_embeds + self.out_pos_embed(action_embeds)
        output = self.decoder(tgt = input_embedding,memory = cond_embedding_geom, tgt_mask = self.tgt_mask.to(self.device))
        output = self.layernorm(output)
        output = self.action_head(output)
        return output
    ...
    def predict_imagegoal_action(self,goal_image,input_images,input_depths,sample_num=16):
        with torch.no_grad():
            ...
            # 新增
            geometry_embed = self.geometry_encoder(np.concatenate((goal_image,input_images[:,-1]),axis=-1)).unsqueeze(0)

            rgbd_embed = torch.repeat_interleave(rgbd_embed,sample_num,dim=0)
            imagegoal_embed = torch.repeat_interleave(imagegoal_embed,sample_num,dim=0)

            # 新增
            geometry_embed = torch.repeat_interleave(geometry_embed,sample_num,dim=0)
            
            noisy_action = torch.randn((sample_num * goal_image.shape[0], self.predict_size, 3), device=self.device)
            naction = noisy_action
            self.noise_scheduler.set_timesteps(self.noise_scheduler.config.num_train_timesteps)
            for k in self.noise_scheduler.timesteps[:]:
                noise_pred = self.predict_noise_geom(naction,k.unsqueeze(0),imagegoal_embed,rgbd_embed, geometry_embed)
                naction = self.noise_scheduler.step(model_output=noise_pred,timestep=k,sample=naction).prev_sample
