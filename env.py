...
class NavEnv():
    ...
    def make_cfg(self) -> habitat_sim.Configuration:
        ...
        ## edited by bo
        if self.args.pitch_deg is not None:
            pitch_rad = math.radians(self.args.pitch_deg)
            camera_orientation = [pitch_rad, 0.0, 0.0]
        else:
...
def hm3d_data_config(args, stage:str='val',
    ...
    ### 清理由于build sam2产生的Hydra实例，否则会报错
    try:
    ...
    with read_write(habitat_config):
        ...
        # 只加载某个 scene（可支持多个）
        if getattr(args, "content_scenes", None):
            # 支持逗号分隔：--content_scenes 00810-CrMo8WxCyVb,00873-bxsVRursffK
            scenes = [s.strip() for s in args.content_scenes.split(",") if s.strip()]
            habitat_config.habitat.dataset.content_scenes = scenes
        else:
            # 如果没传，则默认全量
            habitat_config.habitat.dataset.content_scenes = ["*"]

        #habitat_config.habitat.environment.iterator_options.num_episode_sample = episodes

        habitat_config.habitat.environment.iterator_options.num_episode_sample = -1      # NEW: 使用全部 episodes
        ...
        if args.pitch_deg is not None:
            ...
            habitat_config.habitat.simulator.agents.main_agent.sim_sensors.rgb_sensor.orientation = [pitch_rad, 0.0, 0.0]
            habitat_config.habitat.simulator.agents.main_agent.sim_sensors.depth_sensor.orientation = [pitch_rad, 0.0, 0.0]
...
def mp3d_data_config(args,stage:str='val',
    ...
    with read_write(habitat_config):
        ...
        habitat_config.habitat.simulator.scene_dataset = args.MP3D_SCENE_PREFIX + "/mp3d.scene_dataset_config.json"
        ...
        if "top_down_map_vlnce" in habitat_config.habitat.task.measurements:
            del habitat_config.habitat.task.measurements["top_down_map_vlnce"]
        habitat_config.habitat.task.measurements.update(
        {
            "top_down_map": TopDownMapMeasurementConfig(
                map_padding=3,
